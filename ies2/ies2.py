...
def main():
    ...
    if '*' in input_file:
        args.input = glob.glob(os.path.expanduser(input_file))
        args.output = list(map(autodetect_output_file, input_file))
    else:
        args.input = [input_file]
        args.output = [args.output or autodetect_output_file(args.input[0])]
    ...
    for input, output in zip(args.input, args.output):
        order = None
        if args.order:
            args.order = os.path.abspath(args.order)
            order_file = os.path.join(args.order, os.path.splitext(os.path.basename(input))[0] + '.xml')
            try:
        ...
        if input.endswith('.ies') and output.endswith('.xml'):
            ies_to_xml(input, output, order, dictionary, encoding, args.float)
        elif input.endswith('.xml') and output.endswith('.ies'):
            xml_to_ies(input, output, order, dictionary, encoding, args.float)
        else:
